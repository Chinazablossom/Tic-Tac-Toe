...
each containing three characters (X’s and O’s) to represent a game of tic-tac-toe where all fields of the grid have been filled in.
 ...
 */
private val rows = 3
private val colums = 3
private val player1 = "X"
private val player2 = "O"

fun main() {
    val board = MutableList(rows) { MutableList(colums) { "$player1$player2$player1".random() } }
    for (item in board.indices) {
        println(board[item].joinToString(separator = " "))
    }
}
